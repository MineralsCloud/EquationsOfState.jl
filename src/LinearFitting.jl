...
function energy_volume_expansion(s::FiniteStrain, v0::Real, v::Real, p::Poly, highest_order::Int = degree(p))
    # The zeroth order value plus values from the first to the ``highest_order`.
    p(v) + dot(energy_volume_derivatives(s, v0, v, p, highest_order), getstrain(s, v0, v).^collect(1:highest_order))
end

function energy_volume_derivatives(s::FiniteStrain, v0::Real, v::Real, p::Poly, highest_order::Int)
